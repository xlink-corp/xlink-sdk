...
### <a name="statistics_snapshot_rule">28.设备快照数据端点统计</a>
	...
	依据快照规则下的某个统计规则对设备数据端点进行统计, 包括不同维度的max, min, avg, sum
...
Content

	{
	    ...
	    "list": [
	        {
	            "datapoint": "数据端点下标",
	            "statistic_mode": "统计方式, avg, max, min, sum",
	            "value_list": [
	                {
...
| statistic_fineness | 是 | 统计粒度, 见[附录](#snapshot_statistic_rule_fineness)。 |
| datapoint | 是 | 数据端点下标 |
| statistic_mode | 是 | 数据端点统计方式, 见[附录](#snapshot_statistic_rule_mode) |
| value_list | 是 | 数据端点某个统计方式的统计数值列表 |
| date | 是 | 某种统计维度的自然开始时间, 如统计维度是小时则为2015-09-28 01:00:00, 如统计维度是天则为2015-09-28 00:00:00, 如统计维度是月则为某个月的第一天2015-09-01 00:00:00 |
| value | 是 | 统计维度下的统计数值 |
...
| 字段 | 是否必须 | 描述 |
| --- | --- | --- |
| statistic_mode | 是 | 统计方式, 见[附录](#snapshot_statistic_rule_mode)。 |
| list | 是 | 符合排名的数据 |
| device_id | 是 | 设备id |
| datapoint | 是 | 数据端点下标 |
| value | 是 | 统计维度下的值 |
...
| 字段 | 是否必须 | 描述 |
| --- | --- | --- |
| statistic_dimension | 是 | 统计维度, 见[附录](#snapshot_statistic_rule_fineness)。 |
| list | 是 | 各个数据端点排名的数据 |
| datapoint | 是 | 数据端点下标 |
| position | 是 | 排名 |
