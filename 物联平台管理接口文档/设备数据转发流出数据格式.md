...
5. [SYNC消息](#sync)
6. [APP STATE消息](#app_state)
7. [APP SYNC消息](#app_sync)
...
### **<a name="app_state">6.APP STATE消息</a>**

	设备在上下线时, XLINK平台会根据APP应用(目前仅为耗材应用)配置的接收URL, 以APP STATE消息格式转发至XLINK平台内部插件。
...
### **<a name="app_sync">7.APP SYNC消息</a>**

	在设备发生数据端点变化时, XLINK平台会根据APP应用(目前仅为耗材应用)配置的接收URL, 以APP STATE消息格式转发至XLINK平台内部插件。
