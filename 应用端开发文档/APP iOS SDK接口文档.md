...
说明：APP登录到云端，登录到云端以后，才可以使用云端的功能。
参数：
...
函数：-(int)scanByDeviceProductID:(NSString*)productID;
说明：通过产品ID扫描本地内网设备
参数：
    productID产品ID
返回值：
...
6. sendLocalPipeData
函数：-(int)sendLocalPipeData:(DeviceEntity*)device andPayload:(NSData*)payload;
说明：向内网中的设备发送透传数据
参数：
    device设备实体;
    payload数据值，二进制的。
...
7. sendPipeData
函数：-(int)sendPipeData:(DeviceEntity*)device andPayload:(NSData*)payload;
说明：通过云向设备发送透传数据
参数：
    device设备实体;
    payload数据值，二进制的;
...
8. initDevice
函数：-(int)initDevice:(DeviceEntity*)device;
说明：初始化（更新）某个设备的基本信息，用在从APP缓存设置到SDK中时使用。
参数：
    device设备实体
返回值：
    0：成功;
...
9. probeDevice
函数：-(int)probeDevice:(DeviceEntity*)device;
说明：探测设备状态
参数：
    device设备实体
返回值：
...
10. connectDevice
函数：-(int)connectDevice:(DeviceEntity*)device andAuthKey:(NSString*)authKey;
说明：探测结果异步通过onDeviceProbe回调
参数：
	device设备实体;
	authKey设备授权码;
返回值：
	0成功;
	其他失败;
备注：连接结果通过onConnectDevice回调

11. stop
函数：-(void)stop;
说明：释放SDK，清理本地资源。在退出程序，或者从休眠恢复之后，都需要再次调用stop和start
参数：
返回值：

12. initDeviceList
函数：-(void)initDeviceList:(NSArray*)devices;
说明：初始化设备列表，用在程序重启后，将app缓存的设备列表，初始化到SDK中，用于通讯和其他功能。
参数：devicesDeviceEntity实体的队列
返回值：

13. getAllDevice
函数：-(NSArray*)getAllDevice;
说明：得到所有缓存的设备列表，返回的NSArray中包含的是DeviceEntity对象
参数：
返回值：DeviceEntity实体的队列;
...
3. onGotDeviceByScan
    函数：-(void)onGotDeviceByScan:(DeviceEntity*)device;
    说明：SDK扫描到的设备结果回调;
    参数：device设备实体;
    备注：如果扫描到了多个设备，该回调会多次调用;

4. onSetLocalDeviceAuthorizeCode
函数：-(void)onSetLocalDeviceAuthorizeCode:(DeviceEntity*)device withResult:(int)result withMessageID:(int)messageID;
说明：设置本地设备的访问授权码结果回调
参数：
    device设备实体;
    result设置结果;
...
函数：-(void)onSetDeviceAuthorizeCode:(DeviceEntity*)device withResult:(int)result withMessageID:(int)messageID;
说明：云端设置授权结果回调
参数：
    device设备实体
    result设置结果
...
7. onSendPipeData
函数：-(void)onSendPipeData:(DeviceEntity*)device withResult:(int)result withMessageID:(int)messageID;
说明：发送云端透传数据结果
参数：
    device设备实体;
    result设置结果;
...
函数：-(void)onRecvLocalPipeData:(DeviceEntity*)device withPayload:(NSData*)data;
说明：接收到设备发送过来的透穿消息
参数：
     device设备实体;
     data消息内容;
...
9. onRecvPipeData
函数：-(void)onRecvPipeData:(DeviceEntity*)device withPayload:(NSData*)payload;
说明：接收到云端设备发送回来的透传数据
参数：
    device设备实体;
    data消息内容;
...
函数：-(void)onRecvPipeSyncData:(DeviceEntity*)device withPayload:(NSData*)payload;
说明：接收到云端设备发送的广播透传数据
参数：
    device设备实体;
    data消息内容;
...
12. onConnectDevice
函数：-(void)onConnectDevice:(DeviceEntity*)device andResult:(int)result andTaskID:(int)taskID;
说明：连接设备结果回调
参数：
    device设备实体;
    result设置结果;
...
函数：-(void)onDeviceStateChanged:(DeviceEntity*)device andState:(int)state;
说明：设备上下线状态回调
参数：
    device设备实体;
    state状态;
...
函数：-(void)onDataPointUpdata:(DeviceEntity*)device withIndex:(int)index withDataBuff:(NSData*)dataBuff withChannel:(int)channel;
说明：数据端点数据回调;
参数：
    device设备实体;
    index端点索引;
...
说明：设备是否初始化过，初始化的概念就是设备有没有被设置过授权码，如果没有就需要先设置授权码才可以使用。

2. BOOLisCloud
说明：设备是云端设备还是本地设备，这个值的判定，由connectDevice时确定，如果是云端设备，相应的发送数据和控制指令的函数，是需要调用云端对应的函数，否则的话就需要调用对应的本地函数。

3. -(NSDictionary*)getDictionaryFormatWithProtocol:(int)protocol;
说明：将设备序列化成Dictionary，并且可以用JSON表示出来，JSON格式如下：
