...
* 流程说明：
	1. 页面获取微信JSAPI调用签名；
	2. 页面初始化微信JSAPI；
	3. 调用JSAPI中的configWXDeviceWiFi接口进行AirKiss配置；
	
> <a href="http://iot.weixin.qq.com/wiki/doc/both/微信硬件JSAPI介绍文档1.0.5.pdf">微信iot JSAPI文档</a>
...
* 接口已经包裹在云智易提供的JSSDK中完成，以下列出作为参考。
* 接口总览：
...
| --- | --- | --- |
| {appid} | 是 | 用户在云智易管理台添加的微信应用AppID |
| access_token | 是 | 用户在云智易平台的调用凭证，通过`get_open_id`时获取 |
| open_id | 是 | 微信用户OpenID |
...
> 获取ticket参考：<a href="http://iot.weixin.qq.com/wiki/doc/both/微信硬件JSAPI介绍文档1.0.5.pdf"> 微信iot开发文档中的 getWXDeviceTicket 接口</a>
