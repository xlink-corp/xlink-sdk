...
4.对应回调： XlinkNetListener onLogin(int code) ，详情请参见XlinkNetListener 说明
...
1.说明：判断是否连接上xlink服务器
2.返回值：true/已连接 false/未连接
...
返回值 :
         ...
         2.  key：      端点索引
         3.  value     端点值
         4.  channel： 通道，区分是在局域网直连设备的更新端点，还是云端更新端点、
...
Scan Device ---------isInit()==true-----------连接设备需要提示，让用户输入之前设置过的设备密码----------连接设备成功-----------控制设备


---------isInit()==false-----------设置设备初始密码(设置成功后，自行存储)----------连接设备-------- 连接设备成功---------------控制设备
               
设置设备初始授权码int setDeviceAuthorizeCode(XDevice device, String oldAuthorize, String newAuthorize, SetDeviceAuthorizeListener listener)

参数里oldAuthorize可随便填入4位以上字符串
